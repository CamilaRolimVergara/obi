# 🏅 Olimpíada Brasileira de Informática (OBI) – Resoluções em Python

Este repositório reúne **resoluções comentadas de problemas da OBI**, organizadas por **ano, modalidade, nível e fase**.

📌 **Objetivo:** servir como **material de estudo** para quem está aprendendo programação e praticando lógica e algoritmos. <br>
📅 **Desafio pessoal:** resolver **1 problema da OBI por dia**.


## 📚 Conteúdo

- [OBI 2018](#obi-2018)
- [OBI 2019](#obi-2019)
- [OBI 2020](#obi-2020)
- [OBI 2022](#obi-2022)

## OBI 2018

### 🧠 Modalidade Programação

#### ⭐ Nível Sênior

- **Primeira Fase**
  - [x] Piso da escola
    - 📄 Enunciado: `ProvaOBI2018_f1.pdf`
    - 🐍 Solução: `piso.py`
  - [x] Figurinhas da Copa
    - 📄 Enunciado: `ProvaOBI2018_f1.pdf`
    - 🐍 Solução: `figurinhas.py`
  - [ ] Ilhas

## OBI 2019

### 🧠 Modalidade Programação

#### ⭐ Nível Sênior

- **Primeira Fase**
  - [ ] Calçada imperial
  - [x] A idade de dona Mônica
    - 📄 Enunciado: `ProvaOBI2019_f1.pdf`
    - 🐍 Solução: `idade.py`
  - [ ] Soma
  - [ ] Chuva

## OBI 2020

### 🧠 Modalidade Programação

#### ⭐ Nível Sênior

- **Primeira Fase - Turno A**
  - [x] Acelerador de partículas
    - 📄 Enunciado: `ProvaOBI2020_f1A.pdf`
    - 🐍 Solução: `acelerador.py`
  - [ ] Fissura perigosa
  - [x] Bingo!
    - 📄 Enunciado: `ProvaOBI2020_f1A.pdf`
    - 🐍 Solução: `bingo.py`
  - [x] Paciente zero
    - 📄 Enunciado: `ProvaOBI2020_f1A.pdf`
    - 🐍 Solução: `paciente.py`

- **Primeira Fase - Turno B**
  - [x] Irmãos
    - 📄 Enunciado: `ProvaOBI2020_f1B.pdf`
    - 🐍 Solução: `irmaos.py`
  - [ ] Garamana
  - [x] Camisetas da olimpíada
    - 📄 Enunciado: `ProvaOBI2020_f1B.pdf`
    - 🐍 Solução: `camisetas.py`
  - [ ] Música para todos

## OBI 2022

### 🧠 Modalidade Programação

#### ⭐ Nível Sênior

- **Primeira Fase**
  - [x] Hotel
    - 📄 Enunciado: `ProvaOBI2022_f1.pdf`
    - 🐍 Solução: `hotel.py`
  - [ ] Maior valor
  - [ ] Quadrado mágico
  - [ ] Chuva

## 🚀 Sobre o projeto

Este repositório faz parte do meu **portfólio de estudos em programação**, com foco em:
- lógica
- algoritmos
- leitura de problemas
- escrita de código limpo

Sinta-se à vontade para usar como **material de apoio** ⭐
